...
@section('page-title', 'Edit Alert Rule')

@section('breadcrumb')
    <span class="topbar-sep">/</span>
    <a href="{{ route('glint.alerts.index') }}" class="breadcrumb-link">Alerts</a>
    <span class="topbar-sep">/</span>
    <span class="breadcrumb-current">Edit Rule</span>
@endsection
...
<div class="page-head" style="max-width:720px;margin-left:auto;margin-right:auto">
    <div>
        <div class="page-title">Edit Alert Rule</div>
    </div>
</div>

<div class="panel" style="max-width:720px;margin-left:auto;margin-right:auto"
     x-data="{
         type: '{{ old('type', $rule->type->value) }}',
         channels: {{ json_encode(old('channels', $rule->channels ?? [])) }},
         enabled: {{ old('enabled', $rule->enabled ? '1' : '0') === '1' ? 'true' : 'false' }},
         thresholdHint() {
             const map = {
    ...
    <form method="POST" action="{{ route('glint.alerts.update', $rule->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                ...
                <div class="form-field">
                    <input id="threshold" type="number" name="threshold" value="{{ old('threshold', $oldThreshold) }}"
                ...
                <div class="form-field">
                    <select id="period" name="period" class="input" style="max-width:160px">
                        @foreach($periods as $p)
                            <option value="{{ $p->value }}" @selected(old('period', $oldPeriod) === $p->value)>
                                {{ ucfirst($p->value) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-field">
                    <select id="provider_filter" name="provider" class="input" style="max-width:200px">
                        ...
                        @foreach($providers as $p)
                            <option value="{{ $p }}" @selected(old('provider', $oldProvider) === $p)>{{ ucfirst($p) }}</option>
                        @endforeach
                    </select>
        ...
        <div class="form-section">
            ...
            <div class="form-row" x-show="hasChannel('mail')" x-cloak>
                ...
                <div class="form-field">
                    <input id="mail_to" type="email" name="mail_to" value="{{ old('mail_to', $rule->mail_to) }}"
            ...
            <div class="form-row" x-show="hasChannel('slack')" x-cloak>
                ...
                <div class="form-field">
                    <input id="slack_webhook_url" type="url" name="slack_webhook_url" value="{{ old('slack_webhook_url', $rule->slack_webhook_url) }}"
            ...
            <div class="form-row" x-show="hasChannel('webhook')" x-cloak>
                ...
                <div class="form-field">
                    <input id="webhook_url" type="url" name="webhook_url" value="{{ old('webhook_url', $rule->webhook_url) }}"
                ...
                <div class="form-field">
                    <div class="input-group">
                        <input id="cooldown_minutes" type="number" name="cooldown_minutes"
                               value="{{ old('cooldown_minutes', $rule->cooldown_minutes) }}"
